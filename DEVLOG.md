...
## 프로젝트 개요
- **프로젝트명**: C-PT (C Programming Tutorial) - C언어 전문 학습 플랫폼
...
## 2026-03-25 (Day 1) - 리브랜딩: CMaster → C-PT (C Programming Tutorial)

### 변경 사유
"CMaster"보다 "C Programming Tutorial"이 사이트 목적을 더 명확히 전달. 약칭 **C-PT**, 부제 **C Programming Tutorial**.

### 변경 파일 및 내용

| 파일 | 변경 내용 |
|------|-----------|
| `index.html` | title, og:title, og:site_name, twitter:title → "C-PT" |
| `Navbar.jsx` | 로고: `C-PT` + subtitle `C Programming Tutorial` |
| `Footer.jsx` | 브랜드: `C-PT` + subtitle |
| `navbar.css` | `.brand-master` → `.brand-pt` + `.brand-subtitle` 추가 |
| `footer.css` | `.brand-master` → `.brand-pt` + `.brand-subtitle` 추가 |
| `ko.js` | heroTitle2, whyCMaster, featuresTitle, guide/login subtitle, copyright |
| `en.js` | 동일 키 영문 |
| `Home.jsx` | hero-highlight, 터미널 출력 |
| `Login.jsx` | 브랜드명, 설명, 코드 스니펫 |
| `Guide.jsx` | FAQ "CMaster는 무엇인가요?" → "C-PT는 무엇인가요?" |
| `Certificate.jsx` | certificate-brand |
