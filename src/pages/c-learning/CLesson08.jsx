...
export default function CLesson08() {
  ...
  return (
    <div className="c-lesson-page">
      ...
      <section className="c-lesson-section">
        ...
        <p>{lang === 'en'
          ? 'A struct groups different data types into a single unit. It allows you to create custom data types:'
          : '구조체는 서로 다른 자료형을 하나의 단위로 묶어줍니다. 사용자 정의 자료형을 만들 수 있습니다:'}</p>
        <CodeEditor
...
int main() {
    ...
    return 0;
          ...
          lessonId="c-lesson-08-struct"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'typedef for Cleaner Code' : 'typedef로 깔끔한 코드 작성'}</h2>
        <CodeEditor
          initialCode={`#include <stdio.h>
#include <math.h>
...
int main() {
    ...
    return 0;
          ...
          lessonId="c-lesson-08-typedef"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Struct Pointers & Arrow Operator' : '구조체 포인터와 화살표 연산자'}</h2>
        <CodeEditor
          initialCode={`#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main() {
    ...
    return 0;
          ...
          lessonId="c-lesson-08-pointer"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Union' : '공용체 (union)'}</h2>
        <p>{lang === 'en'
          ? 'A union stores different data types in the same memory location. Only one member is valid at a time:'
          : '공용체는 같은 메모리 위치에 서로 다른 자료형을 저장합니다. 한 번에 하나의 멤버만 유효합니다:'}</p>
        <CodeEditor
          initialCode={`#include <stdio.h>
...
int main() {
    ...
    return 0;
          ...
          expectedOutput="Size of union Data: 4 bytes"
          lessonId="c-lesson-08-union"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Enum & Bit Fields' : '열거형(enum)과 비트 필드'}</h2>
        <CodeEditor
          initialCode={`#include <stdio.h>
...
int main() {
    ...
    return 0;
          ...
          expectedOutput="Today: Wed (value: 3)"
          lessonId="c-lesson-08-enum"
        />
      </section>

      <div className="c-lesson-nav-bottom">
        <Link to="/c-learning/07" className="c-lesson-nav-btn prev">
          <i className="fa-solid fa-arrow-left" />
          {lang === 'en' ? 'Prev: Advanced Pointers' : '이전: 포인터 심화'}
        </Link>
        <Link to="/c-learning/09" className="c-lesson-nav-btn next">
          {lang === 'en' ? 'Next: File I/O' : '다음: 파일 입출력'}
