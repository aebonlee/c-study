...
export default function CLesson07() {
  ...
  return (
    <div className="c-lesson-page">
      ...
      <div className="c-lesson-warning" style={{ padding: '12px 16px', margin: '0 0 20px', background: 'var(--warning-bg, #fff3cd)', color: 'var(--warning-text, #856404)', borderRadius: '8px', fontSize: '0.9rem', border: '1px solid var(--warning-border, #ffc107)', lineHeight: 1.6 }}>
        ...
        {lang === 'en'
          ? 'This lesson uses malloc, free, and other dynamic memory features not supported by the browser compiler (JSCPP). Please use a local compiler like GCC to run these examples.'
          : '이 레슨은 malloc, free 등 브라우저 컴파일러(JSCPP)에서 지원하지 않는 동적 메모리 기능을 사용합니다. GCC 등 로컬 컴파일러에서 실행해 주세요.'}
      </div>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Double Pointers (Pointer to Pointer)' : '이중 포인터 (포인터의 포인터)'}</h2>
        <p>{lang === 'en'
          ? 'A double pointer stores the address of another pointer. This is useful for modifying pointer values inside functions and for 2D dynamic arrays:'
          : '이중 포인터는 다른 포인터의 주소를 저장합니다. 함수 내에서 포인터 값을 수정하거나 2차원 동적 배열에 유용합니다:'}</p>
        <CodeEditor
...
int main() {
    ...
    **dptr = 99;
    ...
    return 0;
          ...
          expectedOutput="a = 10\n*ptr = 10\n**dptr = 10"
          lessonId="c-lesson-07-doubleptr"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Dynamic Memory Allocation: malloc & free' : '동적 메모리 할당: malloc과 free'}</h2>
        <p>{lang === 'en'
          ? 'malloc() allocates memory on the heap at runtime. Unlike stack variables, heap memory persists until explicitly freed:'
          : 'malloc()은 실행 시간에 힙에 메모리를 할당합니다. 스택 변수와 달리, 힙 메모리는 명시적으로 해제할 때까지 유지됩니다:'}</p>
        <CodeEditor
          initialCode={`#include <stdio.h>
#include <stdlib.h>
...
int main() {
    ...
    *numPtr = 42;
    ...
    return 0;
          ...
          lessonId="c-lesson-07-malloc"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'calloc & realloc' : 'calloc과 realloc'}</h2>
        <CodeEditor
          initialCode={`#include <stdio.h>
#include <stdlib.h>
...
int main() {
    ...
    return 0;
          ...
          lessonId="c-lesson-07-calloc"
        />
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Memory Leaks & Best Practices' : '메모리 누수와 모범 사례'}</h2>
        <CodeEditor
          initialCode={`#include <stdio.h>
#include <stdlib.h>
...
int main() {
    ...
    return 0;
          ...
          lessonId="c-lesson-07-leaks"
        />
        <div className="c-lesson-warning">
          <i className="fa-solid fa-triangle-exclamation" />
          <p>{lang === 'en'
            ? 'Common mistakes: forgetting free(), losing pointer references, double free, and using freed memory (dangling pointer). Use tools like Valgrind to detect memory leaks.'
            : '흔한 실수: free() 누락, 포인터 참조 손실, 이중 해제, 해제된 메모리 사용(댕글링 포인터). Valgrind 같은 도구로 메모리 누수를 감지하세요.'}</p>
        </div>
      </section>

      <section className="c-lesson-section">
        <h2>{lang === 'en' ? 'Dynamic 2D Array' : '동적 2차원 배열'}</h2>
        <CodeEditor
          initialCode={`#include <stdio.h>
#include <stdlib.h>
...
int main() {
    ...
    return 0;
          ...
          lessonId="c-lesson-07-2darray"
        />
      </section>

      <div className="c-lesson-nav-bottom">
        <Link to="/c-learning/06" className="c-lesson-nav-btn prev">
          <i className="fa-solid fa-arrow-left" />
          {lang === 'en' ? 'Prev: Pointer Basics' : '이전: 포인터 기초'}
        </Link>
        <Link to="/c-learning/08" className="c-lesson-nav-btn next">
          {lang === 'en' ? 'Next: Structs & Unions' : '다음: 구조체와 공용체'}
